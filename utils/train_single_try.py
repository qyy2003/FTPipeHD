...
from models.BERT.model import SubBERTForClassification
...
def train_single():
    ...
    bert1 = SubBERTForClassification(0, 3, get_model_args())
    bert2 = SubBERTForClassification(4, 9,get_model_args())
    bert3 = SubBERTForClassification(10, 14,get_model_args())
    last_batch=[]
    for iter_id, batch in enumerate(train_dataloader):
        ...
        for key, value in data1.items():
            data10[key] = value
        data10['default'] = data1['default'].detach().clone()
        data10['default'].requires_grad = True
        data11 = {}
        for key, value in data10.items():
            data11[key] = value

        last_batch.append(copy.deepcopy(data10))

        data2 = bert2(data11)
        data20 = {}
        for key, value in data2.items():
            data20[key] = value
        data20['default'] = data2['default'].tolist()
        data20['default']=torch.tensor( data20['default'] )
        ...
        data2_1=bert2(last_batch[0])
        print(iter_id,data3[1])
        # print(data20['default']retain_grad())
        # print(data20['default'].grad)
        li=data20['default'].grad.tolist()
        li=torch.tensor(li)
        print(li._version)
        data2['default'].backward(li)
        data1['default'].backward(data10['default'].grad)
        continue
    # if is_load_cp():
    #     load_epoch = 0
