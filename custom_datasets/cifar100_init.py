...
def init_cifar100():
    ...
    trainset = CIFAR100(cfg.data.train.dataset_path, transform=transform_train, train=True, download=True)
    train_dataloader = torch.utils.data.DataLoader(trainset, batch_size=cfg.data.batch_size, shuffle=True, num_workers=2)

    testset = CIFAR100(cfg.data.val.dataset_path, transform=transform_test, train=False, download=True)
